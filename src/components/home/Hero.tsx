...
export default function Hero() {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <motion.div 
        className="fixed inset-0 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900"
        style={{ opacity: backgroundOpacity }}
      />

      {/* Animated Background Gradient */}
      <motion.div 
        className="fixed inset-0 bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-pink-500/10 animate-gradient-xy"
        style={{ opacity: backgroundOpacity }}
      />
      ...
      <motion.div 
        className="fixed top-20 left-10 text-teal-500/20 text-6xl animate-spin-slow"
        style={{ opacity: backgroundOpacity }}
      >
        <FaDna />
      </motion.div>
      <motion.div 
        className="fixed bottom-20 right-10 text-blue-500/20 text-6xl animate-spin-slow-reverse"
        style={{ opacity: backgroundOpacity }}
      >
        <FaDna />
      </motion.div>
      ...
      >
        ...
        >
          ...
          >
            Explore the fascinating world of biology through interactive learning and hands-on experiments
          </motion.p>
            ...
            {[
              {
                ...
                description: "Hands-on experience with modern lab equipment and techniques"
              },
              {
                icon: FaUsers,
                color: "text-blue-400",
                title: "Collaborative Learning",
                description: "Work with peers on exciting projects and research"
              },
              {
                icon: FaBook,
                color: "text-purple-400",
                title: "Expert Guidance",
                description: "Learn from experienced mentors and biology professionals"
              }
            ].map((feature, index) => (
              <motion.div
                key={index}
                className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 hover:bg-gray-700/50 transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:shadow-teal-500/10"
                whileHover={{ y: -5 }}
                whileTap={{ scale: 0.98 }}
              >
                <feature.icon className={`text-4xl ${feature.color} mx-auto mb-4`} />
          ...
          >
            ...
            <Link href="/competition" className="group">
              <motion.button 
                className="px-8 py-4 bg-gray-700/50 backdrop-blur-sm rounded-full text-white font-semibold text-lg hover:bg-gray-700 transition-all flex items-center gap-2"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
          ...
          >
            {[
              ...
              { value: "20+", label: "Weekly Sessions", color: "text-blue-400" },
              { value: "15+", label: "Lab Projects", color: "text-purple-400" },
